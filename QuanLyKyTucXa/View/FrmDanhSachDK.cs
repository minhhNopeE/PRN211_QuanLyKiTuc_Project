...
{
    public partial class FrmDanhSachDK : Form
    {
        public FrmDanhSachDK()
        {
            InitializeComponent();
        }
        DAO.DAO_SinhVien DAO_SinhVien = new DAO.DAO_SinhVien();
        DAO.DAO_SvDangKy DAO_SvDangKy = new DAO.DAO_SvDangKy();

        private void DanhSachDangKy_Load(object sender, EventArgs e)
        {
            cbMaphong.DataSource = DAO_SinhVien.SelectMaPhong();
            cbMaphong.ValueMember = "maphong";
            dgvSvDangKy.DataSource = DAO_SvDangKy.SelectSinhVien();
        }

        private void btnThem_Click(object sender, EventArgs e)
        {
            if (DAO_SvDangKy.KiemTraPhongDay(cbMaphong.Text) == true)
            {
                MessageBox.Show("Phòng này đã đầy!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            DAO_SvDangKy.InsertSinhVien(txtMasv.Text, txtTensv.Text, cbGioitinh.Text, dtpNgaysinh.Value, txtQuequan.Text, cbKhoa.Text, txtLop.Text, cbMaphong.Text, cbLoaiuutien.Text);
            btnCapnhat_Click(sender, e);
        }

        private void btnXoa_Click(object sender, EventArgs e)
        {
            DAO_SvDangKy.DeleteSinhVienDangKy(txtMasv.Text);
            btnCapnhat_Click(sender, e);
        }
        ...
        private void btnSua_Click(object sender, EventArgs e)
        {
            ...
            DAO_SvDangKy.UpdateSinhVienDangKy(txtMasv.Text, txtTensv.Text, cbGioitinh.Text, dtpNgaysinh.Value, txtQuequan.Text, cbKhoa.Text, txtLop.Text, cbMaphong.Text, cbLoaiuutien.Text);
            btnCapnhat_Click(sender, e);
        }
